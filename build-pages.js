...
const galleryFigs = ids => `<div class="pg-grid">` + ids.map(id => {
  const g = SITE.gallery.find(x => x[0] === id);
...
/* --------------------------------------------------------------- our work -- */
{
  ...
  write('our-work/index.html', shell({
    ...
    body: `<section class="page-head"><div class="wrap">${crumbTrail(crumbs)}
  <h1>Our work</h1>
  <p class="lede">${SITE.gallery.length} photographs of finished roofs, leadwork and jobs in progress from around ${esc(T.TOWN)} and ${esc(T.COUNTY)}.</p>
</div></section>
<section class="sec s-light"><div class="wrap">
  <div class="prose" style="max-width:none">
    ...
    ${galleryFigs(SITE.gallery.map(g => g[0]))}
